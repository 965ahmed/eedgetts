// SRT to TXT
function convertSrtToTxt(srtText) {
    const lines = srtText.trim()
    ...
    lines.forEach(block => {
        ...
        if (parts.length >= 3) {
            const text = parts.slice(2)
                .join(' '); // Combine all lines after the timestamp
...
//EPUB to TXT
async
function convertEpubToTxt(epubBinary) {
    ...
    const toc = await zip.file(toc_path + 'toc.ncx')
        .async('text');
    const parser = new DOMParser();
    const tocDoc = parser.parseFromString(toc, 'application/xml');
    const navPoints = tocDoc.getElementsByTagName('navPoint');
    for (let i = 0; i < navPoints.length; i++) {
        const src = toc_path + navPoints[i].getElementsByTagName('content')[0].getAttribute('src')
            .split("#")[0];
    ...
    JSZip.loadAsync(zipFile)
        ...
        zip.forEach(function(relativePath, file) {
            ...
            if (file_name_toLowerCase.endsWith('.txt')) {
                file.async('text')
                    .then(result => get_text(file.name.slice(0, file.name.lastIndexOf(".")), result, true));
            } else if (file_name_toLowerCase.endsWith('.fb2')) {
                file.async('text')
                    .then(result => get_text(file.name.slice(0, file.name.lastIndexOf(".")), convertFb2ToTxt(result), true));
            } else if (file_name_toLowerCase.endsWith('.epub')) {
                file.async('ArrayBuffer')
                    .then(result => unzip_epub(file, result));
            } else if (file_name_toLowerCase.endsWith('.srt')) {
                file.async('text')
                    .then(result => get_text(file.name.slice(0, file.name.lastIndexOf(".")), convertSrtToTxt(result), true));
            }
        });
    }, function(e) {
...
function unzip_epub(file, file_text) {
    ...
    const epub_file = new File([blob], 'my_epub_file_name.epub', {
        type: 'application/epub+zip'
    });
    convertEpubToTxt(epub_file)
        .then(result => get_text(file.name.slice(0, file.name.lastIndexOf(".")), result, true))
}
