class SocketEdgeTTS {
    ...
    async onSocketMessage(event) {
        ...
        if (typeof data == "string") {
            if (data.includes("Path:turn.end")) {
                ...
                // Process the parts of the Blob and save as MP3
                for (let _ind = 0; _ind < this.audios.length; _ind++) {
                    ...
                    // Find positions of data separator
    ...
    async saveFiles(blob, blockIndex) {
        if (this.start_save == false) {
            ...
            const fileHandle = await new_folder_handle.getFileHandle(this.my_filename + "_block_" + blockIndex + '.mp3', { create: true });
        ...
        if (this.my_uint8Array.length > 0) {
            ...
            if (!this.save_to_var) {                
                ...
                if (save_path_handle ?? false) {
                    await this.saveFiles(blob_mp3, this.indexpart + 1)
                } else {
    ...
    // New method to process and split .srt content
    parseSRT(srtText) {
    ...
    async synthesizeFromSRT(srtText) {
        const blocks = this.parseSRT(srtText);
        this.indexpart = 0;
        
        for (let block of blocks) {
            const text = block.text;
            const filename = `${this.my_filename}_block_${this.indexpart + 1}.mp3`;

            this.update_stat(`Processing block ${this.indexpart + 1}`);

            // Create SSML for this block
            const ssml = this.mkssml(text);

            // Send SSML to the TTS service
            await this.sendTTSRequest(ssml, filename);

            this.indexpart++;
        }
    }

    // Add this method for TTS request handling for SRT blocks
    async sendTTSRequest(ssml, filename) {
        const my_data = this.date_to_string();
        this.socket.send(
            this.ssml_headers_plus_data(
                this.connect_id(),
                my_data,
