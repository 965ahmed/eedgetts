class ProcessingFile {
	...
	getFixPoints(text) {
		...
		// Handle punctuation replacement options
	...
	get_fix_section(sentences) {
		...
		for (let i = 0; i < sentences.length; i++) {
			if (
				...
				sentences[i - 2].trim() === "" &&
				sentences[i - 3].trim() === "" &&
				this.containsPronounceableChars(current_text)
			) {
				current_text += "\n";
			...
			for (let j = 0; j < words.length; j++) {
				let word = words[j];
				if (
					current_text.length + word.length > this.LAST_STRINGS_LENGTH &&
	...
	clear() {
		...
		this.file_names = [];
		this.file_names.push(["Book", 0]); // "Книга" translated to "Book"
		this.full_text = "";
