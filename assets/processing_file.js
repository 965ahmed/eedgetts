...
fix_text = fix_text.replace(/ fix_text = fix_text.replace(/>/g, ")");
...
const regex = new RegExp(match[3], 'g')

const replacement = match[4].replace(/\\r/g, '\r').replace(/\\n/g, '\n')
...
fix_text = fix_text.replaceAll(match_arr.toString(), match_arr[3].toString())
...
const regex = new RegExp('(^|\\s)'+match_arr.toString()+'(?=\\s|$)', 'gi');
[5]
fix_text = fix_text.replace(regex, '$1'+match_arr[3].toString())
...
this.file_names[this.file_names.length-1][3] = this.all_sentences.length
