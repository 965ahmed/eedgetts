...
var srt_blocks = [] // Array to store parsed SRT blocks
...
book_loaded = false
statArea.value = ""
srt_blocks = [] // Clear previous SRT blocks
if (book) {
book.clear()
book = null
}
...
if (file_name_toLowerCase.endsWith('.srt')) {
// Handle SRT file
fileButton.textContent = "Opened";
srt_blocks = parseSRT(reader.result);
stat_str.textContent = `0 / ${srt_blocks.length}`;
textArea.value = "";
statArea.value = "";
// **Iterate through srt_blocks and create SocketEdgeTTS instances**
srt_blocks.forEach((block, index) => {
parts_book.push(
new SocketEdgeTTS(
index, // _indexpart
file.name.slice(0, file.name.lastIndexOf(".")), // _filename
(index + 1).toString().padStart(4, '0'), // _filenum (can be based on index)
"Microsoft Server Speech Text to Speech Voice (" + voice.value + ")", // _voice
String(pitch_str.textContent), // _pitch
rate_str.textContent, // _rate
"+0%", // _volume
block.text, // _text
statArea, // _statArea
threads_info, // _obj_threads_info
(mergefiles.value == 1) ? false : true, // _save_to_var (adjust as needed)
block.index, // _srt_index
block.timecode // _srt_timecode
)
);
statArea.value += `Block ${(index + 1).toString().padStart(4, '0')}: Opened\n`;
});
num_book = srt_blocks.length; // Set num_book to the number of SRT blocks
run_work = true; // Set run_work to true to start processing
add_edge_tts((mergefiles.value == 1) ? false : true); // Start the audio generation
} else if (file_name_toLowerCase.endsWith('.txt')) {
...
blocks.push({ index, timecode, text: text.trim() });
i++; // Skip the empty line between blocks
}
}
return blocks;
}
...
let file_name = book.file_names[file_name_ind]
...
file_name_ind += 1
file_name = book.file_names[file_name_ind]
...
if (!book_loaded && srt_blocks.length === 0) {
num_text += 1
get_text("Text " + (num_text).toString().padStart(4, '0'), textArea.value, false)
} else if (srt_blocks.length > 0) {
// **Iterate through the parsed SRT blocks and create SocketEdgeTTS instances**
srt_blocks.forEach((block, index) => {
parts_book.push(
new SocketEdgeTTS(
index, // _indexpart (using index as part number for now)
fileInput.files.name.slice(0, fileInput.files.name.lastIndexOf(".")), // _filename
(index + 1).toString().padStart(4, '0'), // _filenum (using index + 1 as file number)
"Microsoft Server Speech Text to Speech Voice (" + voice.value + ")", // _voice
String(pitch_str.textContent), // _pitch
rate_str.textContent, // _rate
"+0%", // _volume
...
if (book_map == true && book_map[2] > 0) {
save_merge(book_map[3], book_map[1], book_map[4], book_map[2])
}
}
}
}
...
if (localStorage.getItem('pointsSelect_value')) {
pointsSelect.value = localStorage.getItem('pointsSelect_value')
}
if (localStorage.getItem('pointsType_innerHTML')) {
pointsType.innerHTML = localStorage.getItem('pointsType_innerHTML')
}
if (localStorage.getItem('voice_value')) {
voice.value = localStorage.getItem('voice_value')
}
if (localStorage.getItem('rate_value')) {
rate.value = localStorage.getItem('rate_value')
}
if (localStorage.getItem('pitch_value')) {
pitch.value = localStorage.getItem('pitch_value')
}
if (localStorage.getItem('max_threads_value')) {
max_threads.value = localStorage.getItem('max_threads_value')
}
if (localStorage.getItem('mergefiles_value')) {
mergefiles.value = localStorage.getItem('mergefiles_value')
}
if (localStorage.getItem('rate_str_textContent')) {
rate_str.textContent = localStorage.getItem('rate_str_textContent')
}
if (localStorage.getItem('pitch_str_textContent')) {
pitch_str.textContent = localStorage.getItem('pitch_str_textContent')
}
if (localStorage.getItem('max_threads_int_textContent')) {
max_threads_int.textContent = localStorage.getItem('max_threads_int_textContent')
}
if (localStorage.getItem('mergefiles_str_textContent')) {
mergefiles_str.textContent = localStorage.getItem('mergefiles_str_textContent')
}
if (localStorage.getItem('statArea_style_display')) {
statArea.style.display = localStorage.getItem('statArea_style_display')
}
if (localStorage.getItem('dopSettings_textContent')) {
dopSettings.textContent = localStorage.getItem('dopSettings_textContent')
}
threads_info = { count: parseInt(max_threads.value), stat: stat_str }
}
